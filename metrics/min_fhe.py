...
class minFHE(Metric):
    ...
    def update(self,
        ...
        inds_best = torch.norm(
            pred_topk[torch.arange(pred.size(0)), :, inds_last, :-1] -               # 计算每个样本的最后一个有效预测与目标值之间的欧几里得距离，找到最小距离的预测
            target[torch.arange(pred.size(0)), inds_last, :-1].unsqueeze(-2), p=2, dim=-1).argmin(dim=-1)
        # 累加了所有样本的最佳预测与目标值之间的角度差异的绝对值之和。
        self.sum += wrap_angle(pred_topk[torch.arange(pred.size(0)), inds_best, inds_last, -1] -          # 计算每个样本的最佳预测与目标值之间的角度差异
